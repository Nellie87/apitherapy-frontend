...
type PnlRow    = { period: string; revenue: number; discounts: number; cogs: number; gross_profit: number; expenses: number; net_profit: number };
...
export default function ExpensesPnLReportPage() {
  ...
  const totals = pnl?.totals ?? { revenue: 0, discounts: 0, cogs: 0, expenses: 0, gross_profit: 0, net_profit: 0 };
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-3 xl:grid-cols-6">
        {[
          { label: "Revenue",      value: fmtMoney(Number(totals.revenue ?? 0)),      sub: "",                                   bg: "bg-slate-50 border-slate-200",   color: "text-slate-900" },
          { label: "Discounts",    value: fmtMoney(Number(totals.discounts ?? 0)),     sub: "",                                   bg: "bg-amber-50 border-amber-200",   color: "text-amber-700" },
          ...
          { label: "Expenses",     value: fmtMoney(Number(totals.expenses ?? 0)),      sub: `${expRatioPct.toFixed(1)}% of rev`,  bg: "bg-red-50 border-red-200",       color: "text-red-700"   },
          {
          ...
          <div className={`${S.card} overflow-hidden`}>
            ...
            <div className="divide-y divide-slate-100 max-h-[440px] overflow-y-auto">
              {loading ? <Spinner/> : trend.length === 0 ? (
                ...
                return (
                  <div key={t.period} className="grid items-center gap-4 px-5 py-3 hover:bg-slate-50 transition-colors"
                    style={{ gridTemplateColumns: "1fr 1fr 1fr" }}>
                    <div className="text-sm font-medium text-slate-700">{fmtDate(t.period)}</div>
                    <div className="text-right text-sm font-bold text-slate-900">{fmtMoney(Number(t.total ?? 0))}</div>
                    <div className="text-right">
                      <div className="flex items-center justify-end gap-2">
                        <div className="w-20 h-1.5 rounded-full bg-slate-100 overflow-hidden">
                          <div className="h-full rounded-full bg-red-400" style={{width:`${Math.max(2,pct)}%`}}/>
                        </div>
                        <span className="text-xs text-slate-500 w-9 text-right">{pct.toFixed(1)}%</span>
                      </div>
                    </div>
                  </div>
                );
            ...
            {trend.length > 0 && (
              <div className="border-t border-slate-200 bg-slate-50 px-5 py-3 grid gap-4" style={{ gridTemplateColumns: "1fr 1fr 1fr" }}>
                <span className="text-xs font-bold text-slate-700">Total</span>
                <span className="text-right text-sm font-bold text-slate-900">{fmtMoney(totalExpenses)}</span>
          ...
          <div className={`${S.card} overflow-hidden`}>
            ...
            <div className="divide-y divide-slate-100">
              {loading ? <Spinner/> : topCats.length === 0 ? (
                ...
                const pct = totalExpenses > 0 ? (c.amount / totalExpenses) * 100 : 0;
                return (
                  <div key={c.category} className="grid items-center gap-4 px-5 py-3 hover:bg-slate-50 transition-colors"
                    style={{ gridTemplateColumns: "1fr 1fr 1fr" }}>
                    <div className="flex items-center gap-2">
                      <span className="h-2.5 w-2.5 rounded-full shrink-0" style={{backgroundColor: CAT_PALETTE[i % CAT_PALETTE.length]}}/>
                      <span className="text-sm font-semibold text-slate-800">{c.category}</span>
                    </div>
                    <div className="text-right text-sm font-bold text-slate-900">{fmtMoney(Number(c.amount))}</div>
                    <div className="text-right">
                      <div className="flex items-center justify-end gap-2">
                        <div className="w-20 h-1.5 rounded-full bg-slate-100 overflow-hidden">
                          <div className="h-full rounded-full transition-all duration-500"
          ...
          <div className={`${S.card} overflow-hidden`}>
            ...
            <div className="overflow-x-auto">
              <div className={`${S.tableHead} min-w-[700px]`}
                style={{ gridTemplateColumns: "1.2fr 1fr 1fr 1fr 1fr 1fr 1fr" }}>
                <div>Period</div>
              ...
              <div className="divide-y divide-slate-100 max-h-[440px] overflow-y-auto min-w-[700px]">
                {loading ? <Spinner/> : pnlRows.length === 0 ? (
                  ...
                  <div key={r.period} className="grid items-center px-5 py-3 hover:bg-slate-50 transition-colors text-sm min-w-[700px]"
                    style={{ gridTemplateColumns: "1.2fr 1fr 1fr 1fr 1fr 1fr 1fr" }}>
                    <div className="font-medium text-slate-700">{fmtDate(r.period)}</div>
                    <div className="text-right font-semibold text-slate-900">{fmtMoney(r.revenue)}</div>
                    <div className="text-right text-amber-600">{fmtMoney(r.discounts)}</div>
                    <div className="text-right text-slate-600">{fmtMoney(r.cogs)}</div>
                    <div className="text-right font-semibold text-green-700">{fmtMoney(r.gross_profit)}</div>
                    <div className="text-right text-red-600">{fmtMoney(r.expenses)}</div>
                    <div className={`text-right font-bold ${r.net_profit >= 0 ? "text-green-700" : "text-red-600"}`}>
              ...
              {pnlRows.length > 0 && (
                <div className="border-t border-slate-200 bg-slate-50 grid items-center px-5 py-3 text-sm font-bold text-slate-900 min-w-[700px]"
                  style={{ gridTemplateColumns: "1.2fr 1fr 1fr 1fr 1fr 1fr 1fr" }}>
                  <span>Total</span>
                  <span className="text-right">{fmtMoney(Number(totals.revenue))}</span>
                  <span className="text-right text-amber-600">{fmtMoney(Number(totals.discounts))}</span>
                  <span className="text-right">{fmtMoney(Number(totals.cogs))}</span>
                  <span className="text-right text-green-700">{fmtMoney(Number(totals.gross_profit))}</span>
                  <span className="text-right text-red-600">{fmtMoney(Number(totals.expenses))}</span>
                  <span className={`text-right ${netProfit >= 0 ? "text-green-700" : "text-red-600"}`}>{fmtMoney(netProfit)}</span>
                </div>
              )}
            </div>
          </div>
